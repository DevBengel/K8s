...
# - PHASE 8: lokales helm + optionale Standard-Repos
###############################################################################
...
# ---------------------------- Local deps -----------------------------
for cmd in ssh scp sshpass base64 curl sed awk grep sha256sum tar; do
...
resolve_versions() {
  ...
  [[ "${HELM_VERSION}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || {
    echo "ERROR: Invalid Helm version '${HELM_VERSION}'" >&2
    exit 1
  }
...
echo
echo "================================================="
echo "💻 PHASE 7 – Local kubectl + helm install + kubeconfig"
echo "================================================="
...
echo
echo "== [7E] Current local helm =="
if command -v helm >/dev/null 2>&1; then
  which helm || true
  helm version || true
else
  echo "No local helm currently found"
fi

echo
echo "== [7F] Remove old local helm =="
if [ -x /usr/local/bin/helm ]; then
  echo "Removing old helm from /usr/local/bin/helm"
  sudo rm -f /usr/local/bin/helm
fi

rm -rf /tmp/linux-amd64 /tmp/helm.tar.gz /tmp/helm.sha256sum
hash -r || true

echo
echo "== [7G] Install local helm ${HELM_VERSION} =="
curl -fsSL --retry 3 --retry-delay 1 -o /tmp/helm.tar.gz "https://get.helm.sh/helm-${HELM_VERSION}-linux-amd64.tar.gz"
curl -fsSL --retry 3 --retry-delay 1 -o /tmp/helm.sha256sum "https://get.helm.sh/helm-${HELM_VERSION}-linux-amd64.tar.gz.sha256sum"

EXPECTED_HELM_SHA="$(cat /tmp/helm.sha256sum | awk '{print $1}')"
ACTUAL_HELM_SHA="$(sha256sum /tmp/helm.tar.gz | awk '{print $1}')"

if [[ "${EXPECTED_HELM_SHA}" != "${ACTUAL_HELM_SHA}" ]]; then
  echo "ERROR: Helm checksum mismatch"
  echo "Expected: ${EXPECTED_HELM_SHA}"
  echo "Actual:   ${ACTUAL_HELM_SHA}"
  exit 1
fi
...
echo
echo "== [7H] Verify local helm =="
which helm
helm version

echo
echo "== [7I] Fetch kubeconfig from kube-1 =="
mkdir -p "$HOME/.kube"
chmod 700 "$HOME/.kube"
...
echo
echo "== [7J] Local cluster access test =="
kubectl get nodes -o wide || true
kubectl get pods -A -o wide || true

echo
echo "== [7K] Helm cluster smoke test =="
helm ls -A || true

# ----------------------------- PHASE 8 -------------------------------
echo
echo "================================================="
echo "📚 PHASE 8 – Prepare common Helm repositories"
echo "================================================="
...
echo
echo "Installed Helm repos:"
helm repo list

echo
echo "🎉 BOOTSTRAP COMPLETE"
echo "Local kubectl is ready. Test with: kubectl get nodes -o wide"
echo "Local helm is ready. Test with: helm version"
