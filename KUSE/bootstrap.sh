...
###############################################################################
# bootstrap-k8s-calico-lab-verbose-final.sh
#
# Fixes encountered LAB issues:
# - /dev/disk/by-id missing -> shim-signed postinst fails -> create dir + dummy link
# - debconf/postinst may require a TTY -> SSH forced with -tt
# - old pkgs.k8s.io repos (e.g. v1.28) -> remove then add v1.32
# - robust /etc/hosts update -> base64 transport to preserve newlines
#
# Runs from ANY admin machine with:
#   ssh, sshpass, base64
###############################################################################
...
ssh_bash_stdin() {
  ...
  echo "===================================================================="
  sshpass -p "$PASS" ssh "${SSH_OPTS[@]}" "${USER}@${ip}" "bash -s" <<<"$content"
}
...
###############################################################################
# PHASE 1 – Hostnames & /etc/hosts (robust)
###############################################################################
echo
echo "================================================="
...
for n in "${NODES[@]}"; do
  ...
  read -r -d '' REMOTE_HOSTS_SCRIPT <<SCRIPT || true
set -Eeuo pipefail
...
read -r -d '' REMOTE_INIT_SCRIPT <<SCRIPT || true
set -Eeuo pipefail
...
read -r -d '' REMOTE_KUBECONFIG_SCRIPT <<SCRIPT || true
set -Eeuo pipefail
...
for ip in "$KUBE2_IP" "$KUBE3_IP"; do
  read -r -d '' REMOTE_JOIN_SCRIPT <<SCRIPT || true
set -Eeuo pipefail
...
###############################################################################
# PHASE 5 – Install Calico (kube-1) – YAML via base64
###############################################################################
echo
echo "================================================="
...
read -r -d '' REMOTE_CALICO_SCRIPT <<SCRIPT || true
set -Eeuo pipefail
